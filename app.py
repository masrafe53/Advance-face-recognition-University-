...
def generate_dataset(nbr):
    face_classifier = cv2.CascadeClassifier(r"C:\Users\LocalAdmin\Desktop\Final_Update\resources\haarcascade_frontalface_default.xml")
...
def train_classifier(nbr):
    dataset_dir = r"C:\Users\LocalAdmin\Desktop\Final_Update\dataset"
...
def face_recognition():
    ...
    faceCascade = cv2.CascadeClassifier(r"C:\Users\LocalAdmin\Desktop\Final_Update\resources\haarcascade_frontalface_default.xml")
    clf = cv2.face.LBPHFaceRecognizer_create()
...
def export_attendance():
    ...
    query = f"select a.accs_id, a.accs_prsn, b.prs_name, b.prs_skill, a.accs_added " \
            f"from accs_hist a " \
            f"left join prs_mstr b on a.accs_prsn = b.prs_nbr " \
            f"where date(a.accs_date) = '{selected_date}' " \
            f"order by 1 desc"

    mycursor.execute(query)
    data = mycursor.fetchall()

    df = pd.DataFrame(data, columns=['accs_id', 'accs_prsn', 'prs_name', 'prs_skill', 'accs_added'])
